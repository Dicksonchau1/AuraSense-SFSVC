...
import { nssimContent } from '../content/nssim';
...
export function ProductNSSIMPage() {
  ...
  const { hero, sections, specs } = nssimContent as ProductPageContent;
  ...
  return (
    <main>
      ...
      {specs && specs.length > 0 && (
        <SectionWrapper id="specs" dark>
          <header className="section-header">
            ...
            <p>Reference performance characteristics for NSSIM intelligence pipeline.</p>
          </header>
          <div className="specs-table" role="table" aria-label="NSSIM Specifications">
            {specs.map((spec) => (
              <div
                key={spec.label}
                className="specs-row"
      ...
      <SectionWrapper id="nssim-cta" dark>
        <header className="section-header">
          <h2>Evaluate NSSIM</h2>
          <p>
            Request access to intelligence pipeline documentation, integration guides,
